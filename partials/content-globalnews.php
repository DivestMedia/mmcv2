...
<section class="alternate" id="global_news">
	<div class="container">
		<div class="row">
						...
						<ul class="list-group list-unstyled nav nav-tabs nav-stacked nav-alternate uppercase">
							<?php foreach ($newsHeadlines as $nheadline): ?>
								<li class="list-group-item">
									<a href="<?=$nheadline->link?>"><?=xyr_smarty_limit_chars($nheadline->title->rendered,60)?></a>
								</li>
							<?php endforeach; ?>
						</ul>
					<?php endif; ?>
			...
			<div class="col-sm-9">
				<?php
				$data = file_get_contents_curl(add_query_arg([
					'page' => 1,
					'categories' => 2,
					'per_page' => 10
				], NEWSBASEURL . 'wp-json/wp/v2/posts'));
				$global_news = json_decode($data);

				if(count($global_news)): ?>
				<div class="text-center news-category-labels margin-top-20 margin-bottom-30">
					<a href="<?=NEWSBASEURL?>" class="news-category-link active"><span class="badge badge-green">Categories</span></a>
					<?php
					// $category_tags = get_category_tags(get_category_by_slug('news')->term_id);
					foreach ($category_tags as $key => $cat) {
						...
						if(in_array($cat['slug'],[
							...
							echo '<a href="'. NEWSBASEURL . $cat['link'].'" class="news-category-link"><span class="badge badge-green">'.$cat['name'].'</span></a>';
						}
										...
										if ($posttags) {
											...
											if($posttags[$ftag]){
												...
												<label class="badge badge-green pull-right margin-top-10 margin-right-10">
													<span href="<?=( NEWSBASEURL . 'tags/'.$posttags[$ftag]->slug)?>" class="text-white"><i class="fa fa-fw fa-tag"></i><?=$posttags[$ftag]->name?></span>
												</label>
												<?php
											}
										}else{
											?>
											<label class="badge badge-green pull-right margin-top-10 margin-right-10">
												<span href="<?=NEWSBASEURL?>" class="text-white"><i class="fa fa-fw fa-tag"></i>News</span>
											</label>
											<?php
										}
					...
					<?php if(count($usa_news)): ?>
						<div class="" id="global-news-post-slider-usa">
							...
							<div class="heading-title text-right margin-top-0 link-viewmore-news">
								<a href="<?=NEWSBASEURL?>/tag/usa/"><h4 class="size-15">View more<span> News</span></h4></a>
							</div>
						<?php endif; ?>
					</div>
					...
					<div class="col-md-4">
						...
						<?php if(count($asia_news)): ?>
							<div class="" id="global-news-post-slider-asia">
								...
								<div class="divider margin-bottom-0 margin-top-10"><!-- divider --></div>
								<div class="heading-title text-right margin-top-0 link-viewmore-news">
									<a href="<?=NEWSBASEURL?>/tag/asia/"><h4 class="size-15">View more<span> News</span></h4></a>
								</div>
							<?php endif; ?>
						</div>
						...
						<div class="col-md-4">
							...
							<?php if(count($stocks_news)): ?>
								<div class="" id="global-news-post-slider-stocks">
									...
									<div class="divider margin-bottom-0 margin-top-10"><!-- divider --></div>
									<div class="heading-title text-right margin-top-0 link-viewmore-news">
										<a href="<?=NEWSBASEURL?>/tag/stocks/"><h4 class="size-15">View more<span> News</span></h4></a>
									</div>
								<?php endif; ?>
							</div>
						</div>
